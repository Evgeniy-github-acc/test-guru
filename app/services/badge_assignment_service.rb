class BadgeAssignmentService
  ...
  def complete_category_assign?(category_title)
    ...
    passed_tests = category_tests.user_succeed_tests(@user.id).ids.uniq
   
    passed_tests == category_tests.ids && @user.badges.where(rule: 'complete_category_assign?', rule_param: category_title).empty?
  end

  def complete_level_assign?(level)
    level_tests = Test.level_sort(@test_passage.test.level)
    passed_tests = level_tests.user_succeed_tests(@user.id).ids.uniq

    passed_tests == level_tests.ids && @user.badges.where(rule: 'complete_level_assign?', rule_param: level).empty?
  end
